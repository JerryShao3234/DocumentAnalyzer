...
Because encryption A is limited to 4 values, determining A is straightforward and uses rounding to the nearest value. P is determined using the index of the highlighted element in the array the DFT outputs. In particular, the frequency is equal to the index of the max element divided by the sample rate (which is just array length).   
